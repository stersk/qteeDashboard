...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// РегламентныеЗадания
	U_ПодключаемыеРегламентныеЗадания.ПриСозданииНаСервере(ЭтотОбъект);
	// РегламентныеЗадания
КонецПроцедуры
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	// РегламентныеЗадания
	Если Не Отказ Тогда
		U_ПодключаемыеРегламентныеЗаданияКлиент.ПередЗакрытием(ЭтотОбъект, Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка);
	КонецЕсли;
	// РегламентныеЗадания
КонецПроцедуры
...
// РегламентныеЗадания
&НаКлиенте
Процедура U_ОткрытьНастройкуПодключаемогоРегламентногоЗадания(Команда)
	U_ПодключаемыеРегламентныеЗаданияКлиент.КомандаОткрытьФормуНастройкиРасписания(ЭтотОбъект, Команда);
КонецПроцедуры
// РегламентныеЗадания
...
&НаСервереБезКонтекста
Функция КомандаТестСоединенияНаСервере()
	ТекстОшибки = "";
	
	РезультатЗапроса = U_ЗагрузкаВыгрузкаДанных.ПолучитьДанныеССервераСтатистики("services/shipment/ping");
	Если Не РезультатЗапроса.Успех Тогда
		ТекстОшибки = СтрСоединить(РезультатЗапроса.Ошибки, Символы.ПС);
	КонецЕсли;
...
&НаКлиенте
Процедура КомандаТестСоединения(Команда)
	СохранитьНастройкиСервер();
	ТекстОшибок = КомандаТестСоединенияНаСервере();
	Если ПустаяСтрока(ТекстОшибок) Тогда
		ТекстОшибок = "Соединение успешно";
	КонецЕсли;
