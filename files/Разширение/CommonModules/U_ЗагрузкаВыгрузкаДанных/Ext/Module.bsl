...
Процедура СоздатьДокумент(ПараметрыДокумента)
	...
	Если Не ЗначениеЗаполнено(НайденныйДокумент) Тогда
		...
		ДокументОбъект.ДополнительныеСвойства.Вставить("ЗаписыватьБезВыгрузки", Истина);
...
Функция ПолучитьБаланс() Экспорт 
	СтруктураРезультат = Новый Структура("Дата, Значение");
	
	Результат = ПолучитьДанныеССервераСтатистики("services/metric/get-metric/balance");
	Если Результат.Успех Тогда
		СтруктураРезультат.Значение = Результат.Данные.value;
		СтруктураРезультат.Дата = ПрочитатьДатуJSON(Результат.Данные.date, ФорматДатыJSON.ISO);
		
		Результат.Данные = СтруктураРезультат;
	КонецЕсли;
	
	Возврат Результат;
КонецФункции

Функция ВыгрузитьСтатистикуВФоне() Экспорт 
	ФоновыеЗадания.Выполнить("U_ЗагрузкаВыгрузкаДанных.ВыгрузитьСтатистику",,"ВыгрузкаОнлайнСтатистики", "Выгрузка онлайн-статистики");
КонецФункции
...
Функция ВыгрузитьОтгрузки() 
	...
	Если МассивДанныхОтгрузок.Количество() Тогда
		...
		КонецЕсли;
	Иначе
		РезультатОтсылки = U_ПроцедурыРаботыСФункциями.ПолучитьСтруктуруВозврата();
	КонецЕсли;
	
	Возврат РезультатОтсылки;
КонецФункции

Функция ВыгрузитьОнлайнПлатежи()
	ДанныеКВыгрузке       = Новый Массив;
	ОбработанныеДокументы = Новый Массив;
	
	ДанныеКУдалению = Новый Массив;
	
	СтруктураОтвет = Новый Структура("id, number, date, sum, commissionRate, description");
	Узел = ПланыОбмена.U_ВыгрузкаОнлайнСтатистики.ПолучитьУзел();
	...
	|	U_ОнлайнПлатеж.Дата КАК date,
	|	U_ОнлайнПлатеж.Номер КАК number,
	|	U_ОнлайнПлатеж.Сумма КАК sum,
	|	U_ОнлайнПлатеж.Комиссия КАК commissionRate,
	|	U_ОнлайнПлатеж.Назначение КАК description,
	|	U_ОнлайнПлатеж.ПометкаУдаления КАК ПометкаУдаления
	|ИЗ
	|	Документ.U_ОнлайнПлатеж.Изменения КАК U_ОнлайнПлатежИзменения
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.U_ОнлайнПлатеж КАК U_ОнлайнПлатеж
	|		ПО U_ОнлайнПлатежИзменения.Ссылка = U_ОнлайнПлатеж.Ссылка
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ПометкаУдаления Тогда
			ДанныеКУдалению.Добавить(Выборка.Ссылка);	
		Иначе
			СтруктураВозврат = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СтруктураОтвет);
			...
			ДанныеКВыгрузке.Добавить(СтруктураВозврат);			
			ОбработанныеДокументы.Добавить(Выборка.Ссылка);
		КонецЕсли;
	КонецЦикла;
	
	// Сначала выгрузим данные
	Если ДанныеКВыгрузке.Количество() Тогда
		Результат = ОтправитьДанныеСтатистики("services/invoice/save-all", ДанныеКВыгрузке);
		Если Результат.Успех Тогда
	...
	// Потом удалим помеченные на удаление
	Для Каждого Документ Из ДанныеКУдалению Цикл
		Результат = ОтправитьДанныеСтатистикиКУдалению("services/invoice/delete", Строка(Документ.УникальныйИдентификатор()));
		Если Результат.Успех Тогда
			ПланыОбмена.УдалитьРегистрациюИзменений(Узел, Документ);		
		КонецЕсли;
	КонецЦикла;
...
Функция ОтправитьДанныеСтатистикиКУдалению(ТочкаВхода, ДанныеКУдалению)
	Заголовки = Новый Соответствие;
		
	ПараметрыЗапроса = Новый Структура;
	ПараметрыЗапроса.Вставить("Сервер", 		U_ОнлайнСтатистикаПовтИсп.АдресСерверСтатистики_Получить());
	ПараметрыЗапроса.Вставить("Порт", 			U_ОнлайнСтатистикаПовтИсп.ПортСерверСтатистики_Получить());
	ПараметрыЗапроса.Вставить("АдресРесурса", 	ТочкаВхода + "/" + ДанныеКУдалению);
	ПараметрыЗапроса.Вставить("Пользователь", 	U_ОнлайнСтатистикаПовтИсп.ПользовательСерверСтатистики_Получить());
	ПараметрыЗапроса.Вставить("Пароль", 		U_ОнлайнСтатистикаПовтИсп.ПарольСерверСтатистики_Получить());
	ПараметрыЗапроса.Вставить("Заголовки", 		Заголовки);
	...
	Результат = U_ПроцедурыРаботыСHTTP.Удалить(ПараметрыЗапроса, "JSON");
	Возврат Результат;
КонецФункции
